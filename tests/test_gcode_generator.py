...
def test_output_has_required_header_lines():
    ...
    # IQ ATC header: Filename + Machine comments, then the ( Material Size) block.
    assert "(Filename:  test_job)" in result
    assert "(Machine:  Laguna IQ ATC)" in result
    assert "( Material Size)" in result
    assert "( X= 1524.000, Y= 3048.000," in result   # material size = bed dims
...
def test_output_header_starts_at_n0():
    p = _placed(SINGLE_T2, "A", 39)
    result = generate_master_gcode([p], SETTINGS)
    assert result.splitlines()[0] == "N0 (Filename:  test_job)"
...
def test_exceeding_tool_capacity_raises():
    # IQ Pro tool changer holds 5; a job needing 6 distinct tools must fail.
    six = _nc([(f"T{i}", f"Tool{i}", 0.25) for i in range(1, 7)])
    p = _placed(six, "A", 39)
    with pytest.raises(ValueError, match="tool changer holds only 5"):
        generate_master_gcode([p], SETTINGS)


def test_exactly_five_tools_is_allowed():
